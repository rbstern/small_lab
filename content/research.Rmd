...
Our research covers several topics.
 Some of the main topics are highlighted below.
...
Most prediction methods assume that the target sample (i.e., the sample we want to make predictions) follows the same distribution as the training sample. However, this is often not true. We develop methods that are able to improve predictions in such settings.
...
data available today in several applications, nonparametric methods have been gaining a lot
of attention as they allow one to make less assumption about the data generating process.
We are interested in developing nonparametric
methods that can be used for settings where the number of covariates is also large.
This poses several interesting computational and statistical problems
...
Bayesian methods are known
to provide improved solutions for several
predictions problems because they offer meningful regularization
...
is key to develop meaningful applications.
...
*Check our [Google scholar](https://scholar.google.com.br/citations?hl=pt-BR&user=MGrbI2UAAAAJ) page
for more papers.*
